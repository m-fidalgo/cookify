...
from utils.exceptions import IngredientNotInRecipeException, PreparationStepNotInRecipeException
...
class RecipeSerializer(serializers.ModelSerializer):
  ...
  class Meta:
    model = Recipe
    fields = ('id', 'title', 'time', 'servings', 'difficulty', 'creator',
              'ingredients', 'categories', 'preparation_steps')
    

  def create(self, validated_data):
    request = self.context['request']
    validated_data['categories'] = attempt_json_deserialize(request.data.get('categories'), expect_type=list)
    validated_data['creator_id'] = attempt_json_deserialize(request.data.get('creator'), expect_type=int)
    recipe = super().create(validated_data)

    ingredients_data = attempt_json_deserialize(request.data.get('ingredients'), expect_type=list)
    for data in ingredients_data:
      Ingredient.objects.create(recipe=recipe, **data)

    preparation_steps_data = attempt_json_deserialize(request.data.get('preparation_steps'), expect_type=list)
